...
safe-outputs:
  add-labels:
    max: 3
  steps:
    - name: Pre-sanitize labels from hardcoded Observability allowlist
      uses: actions/github-script@v9
      env:
        GH_AW_AGENT_OUTPUT: ${{ steps.setup-agent-output-env.outputs.GH_AW_AGENT_OUTPUT }}
        # Observability-owned: not a workflow_call input — callers cannot widen this.
...
## add-labels Limitations (Observability merge-ready)

- **Labels**: Max 3 labels per run. Each label max 64 characters. Labels starting with `-` are rejected.
- **Allowlist**: Hardcoded to `oblt-aw/ai/merge-ready` only (not a caller input). All other labels are stripped before processing.
