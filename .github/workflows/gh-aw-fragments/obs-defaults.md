...
Note: gh-aw v0.89.15 merges `report-failed-jobs` from imports into the main workflow `safe-outputs`. Keep the fragment default set to false, but do not repeat it on each importing `gh-aw-*.md`. Do not declare a bare `noop:` on the main workflow; that overrides this fragment and restores `report-as-issue: true`.
