...
- The pull request head is already checked out in `GITHUB_WORKSPACE`. Start by reading the local diff (`git diff`, changed workflow/manifest files) to identify dependency updates. Do **not** stop solely because shell `gh` cannot reach GitHub.
- Shell `gh` is **not** authenticated in the agent sandbox. That does **not** mean GitHub is unreachable.
- When you need GitHub API data (PR metadata, release notes, commit verification), use the read-only **GitHub MCP** or the `github` CLI on `PATH`. Do not use shell `gh` for those reads.
