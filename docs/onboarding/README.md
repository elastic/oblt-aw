...
This folder collects **onboarding** for teams that use **OBLT Agentic Workflows** (`oblt-aw`): the control plane in [elastic/oblt-aw](https://github.com/elastic/oblt-aw), unified client templates (`trigger-oblt-aw.yml` + `oblt-aw.yml`), and per-repository [Control Plane Dashboard](../operations/control-plane-dashboard.md) behavior.
