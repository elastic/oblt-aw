...
7. **Issue comment** — Before finishing, call `add-comment` with the mandatory template in [`gh-aw-onboard-repository`](../workflows/gh-aw-onboard-repository.md) / the agent source (heading `### Onboarding PR checklist`, one line per allowlisted repo, full `https://github.com/.../pull/<n>` or `#aw_…` links, `### Merge order (manual)`, manual-merges note, user-guide URL). Use **real Markdown newlines** — never escaped `\n` in the body. Do **not** invent alternate headings, `owner/repo#n` shorthand, `pulls?q=` search links, or long discovery/pre-commit digressions. For newly requested PRs, use matching `#aw_…` temporary IDs; on retry with already-open PRs, use real URLs from search / `gh pr list`.
