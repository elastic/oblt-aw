...
Automerge splits identities: the `approve` / `gh-aw-mention-in-pr` job always passes empty `github-token-policy` (`GITHUB_TOKEN` / `github-actions[bot]`) because automerge continues via job `needs` and does not need review writes to re-trigger workflows. Automerge merge jobs (`automerge` / merge-fallback) mint a Vault-app token when `shared-token-policy` is non-empty so REST merges run as that app (needed with `pull_request_bypassers` for CODEOWNERS-protected repos; Apps cannot be listed in `CODEOWNERS`); when empty they use `GITHUB_TOKEN`.
