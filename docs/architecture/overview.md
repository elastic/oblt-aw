...
3. **Runtime check** (`check-dashboard`): When the client runs, a `check-dashboard` job runs **before** calling the ingress; fetches the dashboard issue via API, parses checkboxes (`☑ <!-- oblt-aw:workflow-id -->`), outputs `enabled_workflows` as JSON array
