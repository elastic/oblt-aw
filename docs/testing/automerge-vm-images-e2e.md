...
2. **Token policy** — `token-policy-6cd7ac55e207` must exist in `elastic/catalog-info` (bound to `elastic/oblt-aw/.github/workflows/e2e-*.yml@*`, same permissions as `token-policy-oblt-aw-oblt-aw`). Shared role name lives in [`config/e2e.json`](../../config/e2e.json) (`workflow-token-policy`); E2E workflows pass it explicitly to `create-token` (wildcard policies cannot auto-derive). Agentic routes keep using `workflow-token-policy` from [`config/obs/active-repositories.json`](../../config/obs/active-repositories.json) via `aw-prelude`.
