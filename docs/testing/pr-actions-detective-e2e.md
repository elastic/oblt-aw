...
4. **Fail workflow** — [`.github/workflows/e2e-pr-actions-detective-fail.yml`](../../.github/workflows/e2e-pr-actions-detective-fail.yml) runs only on the fixture PR (label + branch + same-repo). Each live run syncs that YAML onto the fixture tip and bumps [`testdata/agentic/pr-actions-detective/e2e-fail-trigger.md`](../../testdata/agentic/pr-actions-detective/e2e-fail-trigger.md) so `pull_request` synchronize fires a real failed run (required so `workflow_run.pull_requests` is non-empty).
