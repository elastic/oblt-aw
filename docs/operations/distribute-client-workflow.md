...
## Distribution Configuration Contract (`active-repositories.json`)
...
- `BASE_REF`: prior commit SHA used to read `active-repositories.json` from git history for removal detection.
...
- Always generates `install` operations for current repositories in `active-repositories.json`.
