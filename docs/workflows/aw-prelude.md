...
Shared reusable prelude for agentic workflows (dashboard read, optional allow lists, token policy).
...
| `token-policy` | Repository token policy from `active-repositories.json` when configured |
