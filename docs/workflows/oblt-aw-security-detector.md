...
- **Call chain**: The distributed client template `oblt-aw-security-detector.yml` grants **`id-token: write`** on the `run-aw` job so the nested `scan` job can call `create-token`.
