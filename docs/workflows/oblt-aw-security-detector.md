...
- **Call chain**: The distributed client template `trigger-oblt-aw-security-detector.yml` grants **`id-token: write`** on the `run-oblt-aw-<event>` job so the nested `scan` job can call `create-token`.
