...
- `approve`: invokes `elastic/ai-github-actions` `gh-aw-mention-in-pr.lock.yml` when `verify` and `check-dependency-collection` pass (Copilot must not call check-run APIs for gating; branch protection handles required checks at merge time). Agent approval uses `GITHUB_TOKEN` (`github-actions`) and does **not** satisfy CODEOWNERS.
- `codeowner-approve`: after `approve`, checks `reviewDecision`. When it is `REVIEW_REQUIRED` (typical when CODEOWNERS approval is still needed), mints an ephemeral token via `elastic/oblt-actions/github/create-token@v1` using the existing `shared-token-policy` (or Vault auto policy) and runs `gh pr review --approve` in the **same** job. When `reviewDecision` is not `REVIEW_REQUIRED`, the job no-ops successfully so repos without CODEOWNERS requirements are unchanged. Ephemeral tokens are not passed across jobs into nested GH-AW workflows.
- `automerge`: runs **pascalgn/automerge-action** with `GITHUB_TOKEN` on the **same** repository as the PR (`PULL_REQUEST` is the PR number), after `codeowner-approve`. Squash-merge when `MERGE_LABELS`, `MERGE_REQUIRED_APPROVALS`, and GitHub mergeability align with branch protection.
...
| `codeowner-approve` | `id-token: write`, `pull-requests: read` (OIDC mint + read `reviewDecision`; approve uses the ephemeral token) |
...
Repositories that require review from Code Owners stay blocked if only `github-actions` has approved. `codeowner-approve` reuses the consumer’s existing `shared-token-policy` (from `aw-prelude` / `active-repositories.json`); no additional catalog-info TokenPolicy is required for this path. Mint and approve happen in the same job because masked installation tokens cannot be passed reliably across jobs into nested `workflow_call` secrets.

**Consumer requirement:** Whenever a repository (or path) requires a CODEOWNERS review for automerge to succeed, add the GitHub App [elastic-vault-github-plugin-prod](https://github.com/apps/elastic-vault-github-plugin-prod) as a code owner for those paths. That app is the identity behind ephemeral tokens from `create-token` / `shared-token-policy`. Listing only a human team (for example `@elastic/observablt-ci`) is not enough—the app must appear in `CODEOWNERS` so its approval counts. Permissions alone do not satisfy CODEOWNERS.
