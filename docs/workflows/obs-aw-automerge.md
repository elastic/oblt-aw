...
**Approve identity:** For most bot authors (Dependabot, Renovate, and others), when `shared-token-policy` is non-empty the nested lock mints an ephemeral Vault-app token and submits the review as that app. When the PR author is already [elastic-vault-github-plugin-prod](https://github.com/apps/elastic-vault-github-plugin-prod), `approve` passes an **empty** `github-token-policy` so the review is submitted with `GITHUB_TOKEN` as `github-actions[bot]` (GitHub rejects Vault self-APPROVE with 422). Org and classic **required review counts** need that non-author approval; classic `pull_request_bypassers` alone do not satisfy org rulesets.
...
- `approve`: invokes `elastic/ai-github-actions` `gh-aw-mention-in-pr.lock.yml` when `verify` and `check-dependency-collection` pass (Copilot must not call check-run APIs for gating; branch protection handles required checks at merge time). Passes `github-token-policy` from `shared-token-policy` except when the author is `elastic-vault-github-plugin-prod[bot]` (empty policy → `GITHUB_TOKEN` / `github-actions[bot]`). Vault approvals for other authors satisfy org/repo **required review counts** (not CODEOWNERS—GitHub Apps cannot be CODEOWNERS).
...
1. **`approve`:** When the PR author is `elastic-vault-github-plugin-prod[bot]`, pass an empty `github-token-policy` so the nested lock uses `GITHUB_TOKEN` and submits APPROVE as `github-actions[bot]` (avoids self-APPROVE 422; satisfies required review counts / org rulesets that classic bypassers cannot skip). For other authors, when `github-token-policy` / `shared-token-policy` is non-empty, the nested lock mints an ephemeral token and submits the review as [elastic-vault-github-plugin-prod](https://github.com/apps/elastic-vault-github-plugin-prod). Empty policy keeps `GITHUB_TOKEN` for nested approve writes.
2. **`automerge` / merge fallback:** When `shared-token-policy` is non-empty, mint a Vault-app token and call the REST merge API as that app. Empty policy uses `secrets.GITHUB_TOKEN` (no CODEOWNERS bypass). `MERGE_REQUIRED_APPROVALS` is always `1` (pascalgn waits for the non-author approval from the `approve` job).

**Consumer requirement (mandatory for newly registered repos):** Keep human/team entries in `CODEOWNERS` when that gate applies, set a non-empty `workflow-token-policy` / `shared-token-policy`, and **always** add the Vault app to classic branch-protection `pull_request_bypassers` in [elastic/observability-github-settings](https://github.com/elastic/observability-github-settings) as part of [repository onboarding](../onboarding/registering-a-repository.md) so a Vault-app merge can bypass CODEOWNERS. Org rulesets that require reviews still need the split approve identity above. Example Terraform:
...
- **Token policy:** `shared-token-policy` from `aw-prelude` / `workflow-token-policy`. Passed as `github-token-policy` into `gh-aw-mention-in-pr.lock.yml` except when the PR author is the Vault app (empty string so approve uses `GITHUB_TOKEN`). Merge jobs still use `shared-token-policy` for Vault `create-token` when non-empty.
