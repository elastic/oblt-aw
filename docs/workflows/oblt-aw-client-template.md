...
- `issue_comment` (`created`) — drives mention-in-issue when comment starts with `/ai` on an issue (not a PR) and the commenter association is `OWNER`, `MEMBER`, or `COLLABORATOR`
