...
Reusable wrapper that calls the locked PR Buildkite Detective workflow in [elastic/ai-github-actions](https://github.com/elastic/ai-github-actions). The distributed client template `trigger-obs-aw-status.yml` calls `obs-aw-event-status.yml`, which runs prelude and routes failed Buildkite `status` events to this workflow when prelude allows `obs:estc-pr-buildkite-detective`.

## Prerequisites

- Triggered via `workflow_call` from `obs-aw-event-status.yml`, which is called by the distributed `trigger-obs-aw-status.yml` client template.
...
- Client template: [obs-aw-client-template.md](obs-aw-client-template.md) — `trigger-obs-aw-status.yml`, registry id `estc-pr-buildkite-detective`
