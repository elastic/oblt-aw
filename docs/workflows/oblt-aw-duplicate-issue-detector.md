...
Reusable wrapper that calls the locked duplicate-issue-detector workflow in [elastic/ai-github-actions](https://github.com/elastic/ai-github-actions). The client template `oblt-aw-duplicate-issue-detector.yml` calls this workflow on `issues` `opened` or `workflow_dispatch` when prelude allows `obs:duplicate-issue-detector`.

## Prerequisites

- Triggered via `workflow_call` from `oblt-aw-duplicate-issue-detector.yml` client templates.
