...
- Nested lock uses optional `GH_AW_GITHUB_TOKEN` / MCP secrets only (same pattern as other `ai-github-actions` agentic locks), and this wrapper does not use `secrets: inherit`. Copilot runs with `github.token`; no repository `COPILOT_GITHUB_TOKEN` secret is required.
