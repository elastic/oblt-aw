...
Reusable wrapper that calls the locked generic issue-triage workflow in [elastic/ai-github-actions](https://github.com/elastic/ai-github-actions). The client template `oblt-aw-issue-triage.yml` calls this workflow on `issues` `opened` when prelude allows `obs:issue-triage`.

## Prerequisites

- Triggered via `workflow_call` from `oblt-aw-issue-triage.yml` client templates.
