...
Reusable wrapper that calls the locked mention-in-issue workflow in `elastic/ai-github-actions`. The client template `oblt-aw-mention-in-issue.yml` calls this workflow on `issue_comment` events when prelude allows `obs:mention-in-issue`.

## Prerequisites

- Triggered via `workflow_call` from `oblt-aw-mention-in-issue.yml` client templates.
