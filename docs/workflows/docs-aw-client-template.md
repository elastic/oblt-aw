# Workflow: Client templates `docs-aw-*.yml`
...
|-----------------|----------|-------------------|
| `docs-aw-ai-menu.yml` | `issues` opened; `issue_comment` edited; `workflow_dispatch` (`issue_number` required) | `docs-aw-ai-menu.yml` |
| `docs-aw-pr-ai-menu.yml` | `pull_request_target` (opened, reopened, synchronize, ready_for_review); `issue_comment` edited; `workflow_dispatch` (`pull_request_number` required) | `docs-aw-pr-ai-menu.yml` |
...
|----------|-----------------|
| `docs-aw-ai-menu.yml` | `actions: read`, `contents: read`, `discussions: write`, `issues: write`, `pull-requests: write` |
| `docs-aw-pr-ai-menu.yml` | Above plus `checks: read`, `id-token: write` |
...
1. Merge distribution PRs that add `docs-aw-ai-menu.yml` and `docs-aw-pr-ai-menu.yml`.
2. Delete `.github/workflows/docs-aw.yml` in the consumer repository.
3. Update Backstage `workflow_ref` / token policies to reference each installed client workflow file.
