...
*Note: Editing the dashboard issue does not trigger this workflow. Dashboard opt-in/opt-out is read at runtime by the ingress (`get_enabled_workflows`); there is no `issues.edited` trigger.*
