...
Routed workflow source: `.github/workflows/gh-aw-automerge.yml` (verify and approve on the PR). Auto-merge enablement runs in `.github/workflows/automerge.yml` via `workflow_dispatch` (triggered with `gh workflow run`) so it is not attached as a long-running check on the PR head commit.
