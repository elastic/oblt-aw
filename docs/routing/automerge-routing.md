...
**`approve` job:** Nested `gh-aw-mention-in-pr` always receives empty `github-token-policy` (`GITHUB_TOKEN` / `github-actions[bot]`). For repos with “Require review from Code Owners”, add the Vault app to classic branch-protection `pull_request_bypassers` and merge as that app (see [obs-aw-automerge.md](../workflows/obs-aw-automerge.md#codeowners-and-ephemeral-tokens)).
