...
Client template chain: `trigger-oblt-aw-pull-request.yml` → `oblt-aw-event-pull-request.yml` → `oblt-aw-automerge.yml`
...
The event-scoped client template includes `labeled` in `pull_request` types (`trigger-oblt-aw-pull-request.yml`).
