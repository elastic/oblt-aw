...
- Observability reusables: `.github/workflows/oblt-aw-<workflow-id>.yml` (each must call `aw-prelude` first; enforced by `scripts/validate_aw_workflow_prelude.py` in CI). Workflows that invoke `gh-aw-*` must also call `aw-resolve-apm-assets.yml` per agent job (`scripts/validate_aw_workflow_resolve_apm_assets.py`).
