...
ACTIONLINT_INSTALLER="/tmp/gh-aw/agent/download-actionlint-${ACTIONLINT_DOWNLOAD_SCRIPT_SHA}.bash"
ACTIONLINT_INSTALLER_URL="https://raw.githubusercontent.com/rhysd/actionlint/${ACTIONLINT_DOWNLOAD_SCRIPT_SHA}/scripts/download-actionlint.bash"
mkdir -p /tmp/gh-aw/agent
curl -fsSL "${ACTIONLINT_INSTALLER_URL}" -o "${ACTIONLINT_INSTALLER}"
echo "${ACTIONLINT_DOWNLOAD_SCRIPT_SHA256}  ${ACTIONLINT_INSTALLER}" | sha256sum -c -
bash "${ACTIONLINT_INSTALLER}" "${ACTIONLINT_VERSION}"
rm -f "${ACTIONLINT_INSTALLER}"
