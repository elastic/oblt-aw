...
ACTIONLINT_INSTALLER_PATH="$(mktemp -p /tmp/gh-aw/agent "download-actionlint-${ACTIONLINT_DOWNLOAD_SCRIPT_SHA}.XXXXXX.bash")"
trap 'rm -f "$ACTIONLINT_INSTALLER_PATH"' EXIT
curl -fsSL "https://raw.githubusercontent.com/rhysd/actionlint/${ACTIONLINT_DOWNLOAD_SCRIPT_SHA}/scripts/download-actionlint.bash" -o "$ACTIONLINT_INSTALLER_PATH"
echo "${ACTIONLINT_DOWNLOAD_SCRIPT_SHA256}  ${ACTIONLINT_INSTALLER_PATH}" | sha256sum -c -
bash "$ACTIONLINT_INSTALLER_PATH" "${ACTIONLINT_VERSION}"
rm -f "$ACTIONLINT_INSTALLER_PATH"
trap - EXIT
