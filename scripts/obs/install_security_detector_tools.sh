...
ACTIONLINT_DOWNLOAD_SCRIPT_PATH="/tmp/gh-aw/agent/download-actionlint-${ACTIONLINT_DOWNLOAD_SCRIPT_SHA}.bash"
trap 'rm -f "$ACTIONLINT_DOWNLOAD_SCRIPT_PATH"' EXIT
curl -fsSL "https://raw.githubusercontent.com/rhysd/actionlint/${ACTIONLINT_DOWNLOAD_SCRIPT_SHA}/scripts/download-actionlint.bash" \
  -o "$ACTIONLINT_DOWNLOAD_SCRIPT_PATH"
echo "${ACTIONLINT_DOWNLOAD_SCRIPT_SHA256}  ${ACTIONLINT_DOWNLOAD_SCRIPT_PATH}" | sha256sum -c -
bash "$ACTIONLINT_DOWNLOAD_SCRIPT_PATH" "${ACTIONLINT_VERSION}"
rm -f "$ACTIONLINT_DOWNLOAD_SCRIPT_PATH"
trap - EXIT
