...
ACTIONLINT_INSTALLER=download-actionlint.bash
curl -fsSL "https://raw.githubusercontent.com/rhysd/actionlint/${ACTIONLINT_DOWNLOAD_SCRIPT_SHA}/scripts/download-actionlint.bash" -o "$ACTIONLINT_INSTALLER"
echo "${ACTIONLINT_DOWNLOAD_SCRIPT_SHA256}  ${ACTIONLINT_INSTALLER}" | sha256sum --check --status
