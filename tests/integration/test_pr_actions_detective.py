"""
Integration checks for obs:pr-actions-detective.
...
WRAPPER_BASENAME = "obs-aw-pr-actions-detective.yml"
LOCK_BASENAME = "gh-aw-pr-actions-detective.lock.yml"
WORKFLOW_ID = "pr-actions-detective"
ORG_KEY = "obs"
RESOLVE_JOB = "resolve-apm-assets"
AGENT_JOB = "pr-actions-detective"

FIXTURE_ROOT = _root / "testdata" / "agentic" / "pr-actions-detective"
...
def _platform_additional_instructions(wrapper: dict) -> str:
    """Return platform-inline text; empty when the wrapper omits the input."""
...
class TestPrActionsWrapperLockWiring:
    ...
    def test_agent_job_passes_resolve_outputs_to_lock_inputs(self) -> None:
        ...
        assert "report-failure-as-issue" not in with_block, (
            "failure-issue policy belongs in obs-defaults / lock source, "
            "not as a wrapper lock input"
        )
        assert "model" not in with_block
        assert "allowed-bot-users" not in with_block
        assert "messages-footer" not in with_block
...
class TestPrActionsResolveFixtures:
    ...
    def test_resolve_maps_additional_instructions_setup_and_layers(self) -> None:
        ...
        assert "PR_ACTIONS_CONSUMER_FRAGMENT_MARKER" in text
        assert "PR_ACTIONS_CONSUMER_INLINE_MARKER" in text
        assert "PR_ACTIONS_COMMON_INLINE_SHOULD_NOT_APPLY" not in text
        assert text.index("PR_ACTIONS_CONSUMER_FRAGMENT_MARKER") < text.index(
            "PR_ACTIONS_CONSUMER_INLINE_MARKER"
        )
    ...
    def test_resolve_without_apm_keeps_empty_platform_only(
...
def test_fixture_tree_is_present() -> None:
    ...
    assert (
        CONSUMER_ROOT / ".github" / "ai" / "fragments" / "pr-actions-fixture.md"
    ).is_file()
